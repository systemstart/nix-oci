...
# Versions of individually load-bearing packages. In nix-oci the Go minor is
# digest-affecting (DESIGN.md), so a bump that moves it moves every image
# digest we produce -- worth seeing before the closure diff.
